...
bool GameManagerBase::connectGameServerByKindID(word wKindID)
{
...
void GameManagerBase::OnGRLogonFinish()
{
	...
	//重入判断
	if (mServerItem && (mServerItem->GetMeUserItem()!=NULL)&&(mServerItem->GetMeUserItem()->GetUserStatus() >= US_SIT))
	{
		//启动进程
		if (!StartGame())
		{
			mServerItem->OnGFGameClose(GameExitCode_CreateFailed);
		}
		return;
	}
	...
	{
		if (mServerItem)
			mServerItem->OnGFGameClose(GameExitCode_Normal);
		return;
	}
	CB_GameLogonFinsh();
}
...
//更新通知
void GameManagerBase::OnGRUpdateNotify(byte cbMustUpdate, const std::string& szDescribeString)
{
...
void GameManagerBase::StartGameReconnect()
{
	...
	{
		cocos2d::log("GameManagerBase::StartGameReconnect() IServerItem::get()->IsService() false!");
