...
void WidgetManager::ButtonAction(Node* pMenu,WidgetUserInfo* pUserInfo,bool bNeedSound)
{
	...
	{
//		SoundFun::playEffect("Audio_Button_Click.mp3");
	}
}
...
{
	QYLabel* pLable = WidgetFun::castNode<QYLabel>(pNode);
	if (kKey == "AnchorPoint")
	{
	...
	if (kKey == "IsTop" && utility::parseBool(kValue))
	{
		pLable->setVerticalAlignment(kCCVerticalTextAlignmentTop);
	}
	if (kKey == "Dimensions")
	{
		cocos2d::Size kSize = utility::parseSize(kValue);
		pLable->setDimensions(kSize);
	}
	if (kKey =="ShowTxt")
	{
	...
	{
		pLable->setSystemFontSize(utility::parseInt(kValue));
	}
	else if(kKey=="Color")
	{
		pLable->setColor(utility::parseColour(kValue));
	}
	else if("OutLine" == kKey)
	{
		...
		pLable->QYEnabelOutLine(kColor,iWidth);
	}
}
...
cocos2d::Node* WidgetManager::createWidgetEdit(WidgetEdit* pInfo)
{
	...
	pTextField->setFontSize(pInfo->kFontSize);
