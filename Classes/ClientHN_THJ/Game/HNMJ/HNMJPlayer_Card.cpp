...
void HNMJPlayer::setChiHuCard(BYTE cbCard)
{
...
BYTE HNMJPlayer::getGangCard(BYTE currentCard)
{
	ASSERT(m_iIdex == 0);

	BYTE cardIndex[MAX_INDEX]={0};
	HNMJ::CGameLogic::Instance().SwitchToCardIndex(m_kHandCardData,m_kHandCardCout,cardIndex);
	bool bValue = HNMJ::CGameLogic::Instance().EstimateGangCard(cardIndex,currentCard);
	if (bValue)
	{
		return currentCard;
	}
	HNMJ::tagGangCardResult kRes;
	HNMJ::CGameLogic::Instance().AnalyseGangCard(cardIndex,m_kWeaveItemArray,m_kWeaveCount,kRes);
